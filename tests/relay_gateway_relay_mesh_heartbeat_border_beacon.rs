...
*/
#[tokio::test]
async fn test_relay_gateway_relay_mesh_heartbeat_border_beacon() {
    common::setup(false).await;

    // Get current time
    let time_now = SystemTime::now();
    
    // Compute duration since UNIX epoch
    let duration_since_epoch = time_now.duration_since(UNIX_EPOCH).unwrap();
    
    let sec = duration_since_epoch.as_secs();        // Get seconds

    // Correct way to add duration to UNIX_EPOCH
    let time_2 = UNIX_EPOCH + Duration::from_secs(sec);
    let duration_since_epoch2 = time_2.duration_since(UNIX_EPOCH).unwrap();
    
    let sec2 = duration_since_epoch2.as_secs();        // Get seconds
    let nsec2 = duration_since_epoch2.subsec_nanos(); // Get nanoseconds

    println!("hi time2: {}.{} seconds", sec2, nsec2);
